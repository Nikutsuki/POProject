...
void Maze::draw(sf::RenderWindow& window, float blockSize) const
{
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            else if(grid[y][x] == Cell::Passage)
            {
                block.setFillColor(sf::Color::White);
            }
            else if (grid[y][x] == Cell::End)
            {
                block.setFillColor(sf::Color::Cyan);
            }
            else if (grid[y][x] == Cell::Door)
            {
                block.setFillColor(sf::Color::Yellow);
            }
            else if (grid[y][x] == Cell::Key)
            {
                block.setFillColor(sf::Color::Black);
            }
            else if (grid[y][x] == Cell::Elixer)
            {
                block.setFillColor(sf::Color::Magenta);
            }
            else if (grid[y][x] == Cell::SpikeFloor)
            {
